...
export function buildTimeline(startHour = 7, hours = 8, seed = 42) {
  ...
  for (let h = 0; h < hours; h++) {
    ...
    while (mLeft > 0) {
      ...
      bands.push({ code, minutes: span });
      mLeft -= span;
    }
    const output = bands
      .filter((b) => b.code === "EF" || b.code === "SL")
      .reduce((a, b) => a + b.minutes * (80 + Math.floor(rand() * 40)), 0);
    const scheduledMin = bands
      .filter((b) => b.code === "EF")
      .reduce((a, b) => a + b.minutes, 0);
    rows.push({
      hour: String(startHour + h).padStart(2, "0") + ":00",
      bands,
      output,
      scheduledMin,
      hasReject: rand() < 0.4,
